...
>
> 对应 PDF：`NA06d迭代法的收敛性.pdf`（共 15 页）
...
### 2.3 松弛法 /* Relaxation Methods */（PDF 第 7–10 页）
...
### 2.4 Jacobi 收敛而 G-S 发散的反例（PDF 第 11–13 页）
...
#### 2.4.1 Jacobi 迭代收敛性分析（PDF 第 12 页）
...
#### 2.4.2 Gauss-Seidel 迭代收敛性分析（PDF 第 13 页）
...
> PDF 第 14 页为"课上习题"标题页，具体习题内容可能需要结合课堂讲授。

---

### 2.6 课程方法总结（PDF 第 15 页）
...
> **PDF 遍历完整性确认**：
> - ✅ 第 1 页：标题页（迭代法的收敛性）
> - ✅ 第 2 页：误差传播方程 $e^{(k)} = B^k e^{(0)}$、范数充分条件 $\|B\|<1$、矩阵序列收敛定义
> - ✅ 第 3 页：核心充要条件定理 $\rho(B) < 1 \Longleftrightarrow B^k \to 0 \Longleftrightarrow$ 迭代收敛
> - ✅ 第 4 页：例题 1 — 用谱半径判断 Jacobi 收敛性（$\rho(B_J) = 0.3592 < 1$）
> - ✅ 第 5 页：例题 1 续 — 用范数判断 Jacobi 收敛性（$\|B_J\|_\infty = 3/4 < 1$）
> - ✅ 第 6 页：充分条件定理汇总（$\|B\|=q<1$、严格对角占优 → Jacobi 和 G-S 均收敛）
> - ✅ 第 7 页：松弛法引入 — 加权平均思想、松弛因子 $\omega$
> - ✅ 第 8 页：松弛法分类（低松弛 $\omega<1$ / G-S $\omega=1$ / SOR $\omega>1$）、松弛法收敛定理
> - ✅ 第 9 页：收敛速度分析 — $\rho(B)$ 决定收敛速度、特征向量展开、$e^{(k)} \sim [\rho(B)]^k$
> - ✅ 第 10 页：例题 2 — 最优松弛因子 $\omega_{opt} = -1/2$
> - ✅ 第 11 页：例题 3 — Jacobi 收敛但 G-S 发散的反例（方程组）
> - ✅ 第 12 页：例题 3 续 — Jacobi 分析（$\rho(B_J)=0$，特征值全零，收敛）
> - ✅ 第 13 页：例题 3 续 — G-S 分析（$\rho(B_{GS})=2>1$，发散）
> - ✅ 第 14 页：课上习题标题页
> - ✅ 第 15 页：课程方法总结（LU/Crout/平方根法/约当消去法）
>
> **无任何知识点缺省缺漏。**
