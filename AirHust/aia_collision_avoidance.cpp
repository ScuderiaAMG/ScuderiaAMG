...
float vel_collision_body[2];                                         //躲避障碍部分速度
...
int main(int argc, char **argv)
{
    ...
    vel_collision_body[0]= 0;
    vel_collision_body[1]= 0;

    vel_collision_ENU[0] = 0;
    vel_collision_ENU[1] = 0;


    //vel_sp_body[0]= 0;//这里用不到了
    //vel_sp_body[1]= 0;
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>Main Loop<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while (ros::ok())
    {
        ...
        Command_now.command = Move_ENU;     //世界系下移动
...
//生成一个随机的速度
float angle=rand()/RAND_MAX*2*3.1415926;
float speed=rand()/RAND_MAX*0.5;
float  vel_random[2];

for(int i = 0; i < 2; i++)
{
    vel_random[i]=speed*cos(angle-3.1415926*i/2);
}

    //3. 计算追踪速度
    vel_track[0] = p_xy * (target_x - pos_drone.pose.position.x)+vel_random[0];
    vel_track[1] = p_xy * (target_y - pos_drone.pose.position.y)+vel_random[1];

    //速度限幅
    for (int i = 0; i < 2; i++)
    {
        vel_track[i] = satfunc(vel_track[i],vel_track_max*cos(angle_c/180*3.1415926-3.1415926*i/2));
    }
    vel_collision_body[0]= 0;
    vel_collision_body[1]= 0;
    ...
    if(flag_collision_avoidance.data == true)
    {
        ...
        {
            //对速度不做限制
            vel_collision_body[0] = vel_collision_body[0] + 0;
            vel_collision_body[1] = vel_collision_body[1] + 0;
        ...
        if(distance_c <= R_inside )
        {
         ...
         float adjustment_factor = F_c / distance_c;
        vel_collision_body[0] -= (distance_cx > 0 ? 1 : -1) * adjustment_factor;
        vel_collision_body[1] -= (distance_cy > 0 ? 1 : -1) * adjustment_factor;
        //避障速度限幅
        for (int i = 0; i < 2; i++)
        {
            vel_collision_body[i] = satfunc(vel_collision_body[i],vel_collision_max*cos(angle_c/180*3.1415926-3.1415926*i/2));
        }
    }
    rotation_yaw(Euler_fcu[2], vel_collision_body, vel_collision_ENU);
    vel_sp_ENU[0] = vel_track[0] + vel_collision_ENU[0];//***********************************************这里track是世界坐标系的，collision是机体系的-----转换为世界系
    vel_sp_ENU[1] = vel_track[1] + vel_collision_ENU[1]; //dyx
    ...
    for (int i = 0; i < 2; i++)
    {
        vel_sp_ENU[i] = satfunc(vel_sp_ENU[i],vel_sp_max*cos(angle_c/180*3.1415926-3.1415926*i/2));
    }
}
...
void printf()
{
    ...
    cout << "vel_collision_ENU_x : " << vel_collision_ENU[0] << " [m/s] "<<endl;
    cout << "vel_collision_ENU_y : " << vel_collision_ENU[1] << " [m/s] "<<endl;
