...
> 说明：分子中加入 $\left(\dfrac{1}{8.33}s + 1\right)$ 是为了在 $G_c(s) = G_K(s)/G_0(s)$ 中产生一个零点，用以抵消原系统在 $\omega_a = 8.33$ 处的极点。
...
> 校正装置是一个**滞后-超前**网络：$(0.25s+1)/(1.33s+1)$ 为**超前部分**（高频补偿），$(0.12s+1)/(0.022s+1)$ 为**滞后部分**（低频提升）。
...
$$G_K(s) = \frac{25\left(\dfrac{s}{2.5} + 1\right)}{s^2(\cdots)}$$
