...
int main(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    ROS_WARN("Completed circle in %.2f seconds", d_time);
    ...
    switch (mission_num)
    {
      ...
      //绕圈
      case 3:
        // if(circle(test_x, test_y, 8.0))
        if(circle_advanced(test_x, test_y, M_PI/50))
        {
          mission_num = 4;
          d_time = (ros::Time::now() - last_request).toSec();
          ROS_WARN("Completed circle in %.2f seconds", d_time);
        }
        break;
