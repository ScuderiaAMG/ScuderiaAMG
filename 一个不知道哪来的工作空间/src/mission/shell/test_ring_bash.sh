...
tmux send-keys -t ros_session:0.1 'sleep 3; source ~/endurance_ws/devel/setup.bash; roslaunch bringup location.launch' C-m
...
tmux send-keys -t ros_session:0.3 'sleep 5; source ~/endurance_ws/devel/setup.bash; roslaunch yolov8_ros ros_predict.launch' C-m
...
# 第二窗口（相机和检测）
# --------------------
# 创建第二窗口，并在初始 pane (1.0) 运行第一个任务
tmux new-window -t ros_session:1 -n camera_detection
tmux send-keys -t ros_session:1.0 'sleep 5; rosrun web_video_server web_video_server' C-m
...
tmux send-keys -t ros_session:2.2 'sleep 7; source ~/endurance_ws/devel/setup.bash; roslaunch mission cross.launch' C-m
