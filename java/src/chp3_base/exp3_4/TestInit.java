...
public class TestInit {                                  // 公共类 TestInit
    int x;                          // 实例变量 x（成员变量），未显式赋值，自动初始化为 int 类型的默认值 0
    public static void main(String[] args) {             // main 方法：程序入口
        TestInit init = new TestInit();                  // 创建 TestInit 对象，init.x（实例变量）被默认初始化为 0
        ...
        }                              // if 语句结束（注意：没有 else 分支，即 x <= 50 时 y 未被赋值）
        z = x + y + init.x;            // 尝试计算：x + y + init.x；若 x <= 50 则 y 未初始化，编译报错！
