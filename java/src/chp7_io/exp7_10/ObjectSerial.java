...
class Employee implements Serializable {     // 定义 Employee 类，实现 Serializable 接口使其可被序列化
    int id;                                   // 员工编号
    String name;                              // 员工姓名
    int age;                                  // 员工年龄
    int salary;                               // 员工薪水
    ...
    public Employee() {}                      // 默认无参构造方法（反序列化时可能需要）

    public Employee(int id, String name, int age, int salary,   // 带参构造方法
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {  // 自定义序列化方法（私有，JVM 自动调用）
        Date savedDate = new Date();             // 获取当前日期时间（此时为序列化时间）
        ...
        out.writeInt(savedDate.getYear());       // 额外写入当前年份（Date.getYear() 返回年份-1900）
    }                                            // writeObject 方法结束
    ...
    private void readObject(ObjectInputStream in) throws IOException {   // 自定义反序列化方法（私有，JVM 自动调用）
        Date readDate = new Date();              // 获取当前日期时间（此时为反序列化时间）
 ...
 */
public class ObjectSerial {                  // 定义 ObjectSerial 公有类（对象序列化主程序）
    ...
    public static void main(String args[]) throws IOException,   // 程序入口 main 方法
        ...
        employ = (Employee) fin1.readObject();  // 反序列化恢复 Employee 对象（自动调用自定义的 readObject 方法）
