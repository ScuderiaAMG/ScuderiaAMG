...
class EmployeeExtern {                       // 定义 EmployeeExtern 基类（员工信息类，未实现任何序列化接口）
    // 字段声明：将被显式保存的成员数据
    int id;                                   // 员工编号
    String name;                              // 员工姓名
    int age;                                  // 员工年龄
    ...
    public EmployeeExtern() {}                // 默认无参构造方法（Externalizable 反序列化时必需）

    EmployeeExtern(int id, String name, int age, int salary, String hireDay,   // 带参构造方法
 ...
 */
class ManagerExtern extends EmployeeExtern implements Externalizable {  // ManagerExtern 继承 EmployeeExtern 并实现 Externalizable
    String position;                          // 管理职位（子类新增字段）

    public ManagerExtern() {}                 // 默认无参构造方法（Externalizable 反序列化时必需）

    ManagerExtern(int id, String name, int age, int salary, String hireDay,   // 带参构造方法
    ...
    public void writeExternal(ObjectOutput out) throws IOException {  // 外部化序列化方法（必须实现）
        Date savedDate = new Date();             // 获取当前日期时间
        ...
        out.writeInt(savedDate.getYear());       // 额外写入当前年份（用于年龄自动更新）
    ...
    public void readExternal(ObjectInput in) throws IOException,   // 外部化反序列化方法（必须实现）
        ...
        Date readDate = new Date();              // 获取当前日期时间（反序列化时刻）
        ...
        age = age + (readDate.getYear() - savedYear);  // 自动更新年龄：原年龄 +（当前年份 - 保存年份）
    }                                            // readExternal 方法结束
 ...
 */
public class ObjectTest {                    // 定义 ObjectTest 公有类（外部化序列化测试主程序）
    ...
    public static void main(String args[]) { // 程序入口 main 方法（此处未声明抛出异常，使用 try-catch 处理）
        ...
        try {                                   // try 块开始，捕获反序列化过程中的异常
            ObjectInputStream fin = new ObjectInputStream(          // 创建 ObjectInputStream
                                            new FileInputStream("data2.ser"));
            // 注意：反序列化路径为 "data2.ser"（相对根目录，与写入路径不同）

            manager = (ManagerExtern) fin.readObject();  // 反序列化恢复对象（自动调用 readExternal 方法）
        } catch (Exception e) {                 // 捕获所有异常
            System.out.println(e);              // 直接将异常信息打印到控制台
        }                                       // try-catch 块结束
