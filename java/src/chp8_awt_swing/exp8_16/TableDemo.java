...
public class TableDemo extends JFrame {                     // 继承 JFrame 创建自定义窗口类

    private boolean DEBUG = true;                           // DEBUG 开关：true 时在控制台输出调试信息

    public TableDemo() {                                    // 构造方法：初始化表格模型和界面
    ...
    class MyTableModel extends AbstractTableModel {         // 定义内部类，继承抽象表格模型
        ...
        final Object[][] data = {                           // 二维 Object 数组，支持多种数据类型
            {"王山", "200101", "计算机科学",                // 第1行：姓名、学号、专业
             "男", new Integer(20), new Boolean(false)},    // 性别、年龄（Integer）、是否在校（Boolean）
            {"李立明", "200103", "计算机科学",              // 第2行
             "男", new Integer(29), new Boolean(true)},
            {"李丽", "200204", "电子工程",                  // 第3行
             "女", new Integer(21), new Boolean(false)},
            {"王红", "200208", "自动化",                    // 第4行
             "女", new Integer(25), new Boolean(true)},
            {"张阿宝", "200301", "机械设计",                // 第5行
             "男", new Integer(31), new Boolean(false)}
        };
        ...
        public Class getColumnClass(int c) {                 // 获取指定列的数据类型
        ...
        public void setValueAt(Object value, int row, int col) { // 设置指定单元格的值（由表格编辑触发）
            ...
            if (data[0][col] instanceof Integer             // 如果该列第1行是 Integer 类型
                ...
                try {                                       // 尝试将 value 转为 Integer
                    data[row][col] = new Integer(value.toString()); // 通过字符串转换为 Integer
                    fireTableCellUpdated(row, col);         // 通知表格该单元格数据已更新（刷新界面）
                } catch (NumberFormatException e) {         // 如果转换失败（如输入了非数字）
                    JOptionPane.showMessageDialog(TableDemo.this, // 弹出错误对话框
    ...
    public static void main(String[] args) {                // 主入口方法
        TableDemo frame = new TableDemo();                  // 创建 TableDemo 窗口对象
        frame.pack();                                       // 自动调整窗口大小
