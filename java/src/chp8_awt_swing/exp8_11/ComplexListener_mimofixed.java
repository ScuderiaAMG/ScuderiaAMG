...
public class ComplexListener_mimofixed implements                     // 定义类，同时实现3个监听器接口
    ...
    public ComplexListener_mimofixed() {                              // 构造方法：初始化所有组件并注册事件监听器
    ...
    public static void main(String args[]) {                // 主入口方法
        ComplexListener_mimofixed two = new ComplexListener_mimofixed();        // 创建 ComplexListener 实例（构造方法自动完成所有初始化）
        System.out.println("ComplexListener created: " + two); // 使用two变量，消除未使用变量警告
    }                                                       // main 方法结束
