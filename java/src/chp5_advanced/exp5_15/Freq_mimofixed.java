...
public class Freq_mimofixed {                                        // 主类（public），实现单词频率统计
    public static void main(String args[]) {               // 主方法：程序执行的入口点
        ...
        for (String a : words) {                           // 增强for-each循环：遍历words数组中的每个单词
            ...
            if (freq == null) {                            // 如果频率为null，说明该单词第一次出现
                freq = Integer.valueOf(1);                     // 初始频率设置为1，使用valueOf替代废弃的构造函数
            } else {                                       // 否则，该单词已经出现过
                freq = Integer.valueOf(freq.intValue() + 1);   // 在原有频率基础上加1（手动拆箱intValue() + 1再装箱），使用valueOf替代废弃的构造函数
            }
