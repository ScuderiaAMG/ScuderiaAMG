...
public class ListIteratorDemo_mimofixed {                            // 主类（public），演示ListIterator的用法
    public static void main(String[] args) {               // 主方法：程序执行的入口点
        ...
        for (int i = 1; i < 5; i++) {                      // 循环变量i从1到4（不包含5）
            list.add(Integer.valueOf(i));                      // 将i包装为Integer对象后添加到列表末尾（list变为[1,2,3,4]），使用valueOf替代废弃的构造函数
        }
        ...
        listIter.add(Integer.valueOf(0));                      // 在迭代器当前位置（列表头部）之前插入元素0（list变为[0,1,2,3,4]），使用valueOf替代废弃的构造函数
        ...
        if (listIter.hasNext()) {                          // 检查是否还有下一个元素（当前游标位于索引1之前？注意：add后游标位置变了）
            ...
            listIter.set(Integer.valueOf(9));                  // 将刚刚返回的元素（索引1处的元素）修改为9（list变为[0,9,2,3,4]），使用valueOf替代废弃的构造函数
