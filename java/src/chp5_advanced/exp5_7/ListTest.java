...
public class ListTest {                                    // 主类（public），演示原始类型集合的使用
    public static void main(String[] args) {               // 主方法：程序执行的入口点
        // 注意：列表中只应存放Integer类型的对象
        List listofInteger = new LinkedList();             // 创建LinkedList集合（原始类型/Raw Type），可以存放任何Object类型的对象
        listofInteger.add(new Integer(2000));              // 向集合中添加一个Integer对象（值为2000）
